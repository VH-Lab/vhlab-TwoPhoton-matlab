...
switch command,
	case 'NewWindow',
		...
		popup = txt; popup.style = 'popupmenu'; popup.Callback = mycallback;
		...
		uicontrol(button,'position',[10+65+5+410 605+sh 60 20],'string','Update','Tag','UpdateBt');
		uicontrol(button,'position',[10+65+5+410 580+sh 60 20],'string','Choose Dir','Tag','ChooseDirBt');
		...
		uicontrol(popup,'position',[10 75+sh 120 20],'String',{'New:','---','New blank stack','New Auto-filled stack'}',...
			'Tag','NewStackMenu');
		%uicontrol(button,'position',[10 75+sh 70 20],'String','New','Tag','NewStackBt');
		uicontrol(button,'position',[10+120+5 75+sh 45 20],'String','Edit','Tag','EditStackBt');

		ud.ds = dirstruct(experdir);
		set(fig,'userdata',ud);
		twophotonbulkload('UpdateBt',fig);
	case 'NewStackMenu',
		menuItem = get(ft(fig,'NewStackMenu'),'value');
		allMenuStrings = get(ft(fig,'NewStackMenu'),'string');
		menuString = allMenuStrings{menuItem};
		set(ft(fig,'NewStackMenu'),'value',1);
		switch menuString,
			case 'New blank stack',
				twophotonbulkload('NewStackBt',fig);
			case 'New Auto-filled stack',
				twophotonbulkload('NewAutoFilledStackBt',fig);
		end;
	case 'ChooseDirBt'    
		dirname=uigetdir;
		set(ft(fig,'PathnameEdit'),'string',dirname)
		cd(dirname)
		twophotonbulkload('UpdateBt',fig);
	case 'UpdateBt',
	...
	case 'NewAutoFilledStackBt',
		analyzetpstack_newautofilledstack(ud.ds);
	case 'EditStackBt',
