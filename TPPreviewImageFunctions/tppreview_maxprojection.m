function [ims,channels] = tppreview_maxprojection(dirnames, tpparams, channellist, parameters, shortname, dirname)
...
for i=1:length(channels),
	...
	if ~isempty(fnameparameters{i}),
		ims{i} = tp_preview_maxprojection_meat(dirnames,tpparams,{fnameparameters{i}},channel,parameters.firstFrames,parameters.numFrames);
	end;
end;

function im = tp_preview_maxprojection_meat(dirnames,params,fnameparameters,channel,firstFrames,numberFrames)
...
%if size(im,3)>1, im = max(double(im),[],3); else, im = double(im); end;
