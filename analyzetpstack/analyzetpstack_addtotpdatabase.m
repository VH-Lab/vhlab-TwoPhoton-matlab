...
for i=1:length(listofcells),
    listofcells(i); 
	% step 1 - make a list of associates for the analyzetpstack info
	...
	assoc = [assoc myassoc('analyzetpstack labels',{listofcells(i).labels})];
	    % here should add information about drift
	for j=1:2:length(varargin), % assign additional associates, if any
		assoc = [assoc myassoc(varargin{j},varargin{j+1})];
	end;

	% step 2 - add any global associates
	tpassociatelistglobals;
	for j=1:length(tpassociatelist), assoc = [assoc tpassociatelist(j)]; end;

	% step 3 - now create the new record for the cell

	mymd = measureddata([],'',''); 
	for j=1:length(assoc), mymd = associate(mymd,assoc(j)); end;

	% step 4 - get associates related to the stimuli
