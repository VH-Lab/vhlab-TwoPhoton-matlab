...
%
%dirname = 'W:\2photon\ferretdirection\oculartransfer\2009-12-07\t00010',
...
if isempty(fname),
    v3 = 1;
    fname = dir([tpdirname filesep '*Cycle001_CurrentSettings_Ch' int2str(channel) '_000001.tif']);
    if isempty(fname),
        error(['Could not create preview image: ' tpdirname filesep '*Cycle001_Ch' int2str(channel) '_000001.tif does not exist.']);
    end;
end;
...
fnameparams = tpfnameparams(tpdirname,channel,params);
...
if v3, extrastring = '_CurrentSettings'; else, extrastring = ''; end;
for i=1:numFrames,
    im = cat(1,im,reshape(imread(...
        [tpdirname filesep fnameprefix 'Cycle' sprintf('%.3d',ffile(n(i),1)) ...
        extrastring '_Ch' int2str(channel) '_' sprintf('%.6d',ffile(n(i),2)) '.tif']),...
