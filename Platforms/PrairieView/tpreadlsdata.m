function [data, t,frametimes, roidata] = tpreadlsdata(dirname, intervals, pixelinds, mode, channel)

% TPREADLSDATA - Reads twophon linescan data
%
%  [DATA, T] = TPREADLSDATA(DIRNAME, INTERVALS, PIXELINDS, MODE)
%
...
% frames for linescans are the 1000xnum pixels tif files;
...
for j=1:size(intervals,1),
	...
	for f=f0:f1,
		...
		if (currScanline_period__us_ ~= params{dirid}.Main.Scanline_period__us_) || ...
			...
			%update pixeltimes, time within each frame that each pixel was
			%recorded
		...
		t_ = pixeltimes + frametimes(f);
		mydriftusage = [];
		if length(drls)>=f, mydriftusage = drls{f}; end;
		subpixelinds = findroiinlinescan([params{dirid}.Main.fullImagePixelsPerLine params{dirid}.Main.fullImageLinesPerFrame], ... % raster size
				 	  size(ims{f}), ... % linescan image size 
					  params{dirid}.Linescanpoints, ... % linescan points
					  pixelinds, ... % pixel indexes in raster image
					  mydriftusage); % drift
		for i=1:length(pixelinds),
			for II=1:length(subpixelinds{i}),
				thepixelinds = subpixelinds{i}{II};
				% thepixelinds needs to exist
				try,
					...
					thepixelinds,
					disp(['This could possible arise from an aborted recording.  Right now this code cannot deal with aborted recordings.']);
				end;
				...
				% breakout into "frames", or separate recording intervals
				if mode==1,
