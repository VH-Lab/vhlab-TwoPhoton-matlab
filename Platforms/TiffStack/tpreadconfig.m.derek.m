...
for i=1:params.Main.Total_cycles,
    ...
    numFrames = length(theimfinfo);
    if isfield(theimfinfo(1),'ImageDescription')&numel(theimfinfo)==1,
        if ~isempty(theimfinfo(1).ImageDescription),
            numFrameString=regexp(theimfinfo(1).ImageDescription,'images=(\d*)','tokens');
            if ~isempty(numFrameString),
                numFrames = str2double(numFrameString{1}{1});
            end;
        end;
    end;
	num_of_images = numFrames;
