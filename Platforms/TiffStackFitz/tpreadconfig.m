...
for i=1:length(extensionlist),
	datafile = dir([tpdirname filesep 'stack_c1_*' extensionlist{i} ]);
...
for i=1:params.Main.Total_cycles,
	...
	num_of_images = 0;
	for c=1:4, % try up to 4 channels
		theimfinfo{c} = [];
		filename_here = datafile(i).name;
		filename_here(8) = int2str(c);
		try,
			theimfinfo{c} = imfinfo([tpdirname filesep filename_here]);
			num_of_images = length(theimfinfo{c});
		end;
	end;
